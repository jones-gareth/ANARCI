...
class CustomInstallCommand(install):
   def run(self):
       ...
       sitepackages = [d for d in site.getsitepackages() if d.endswith('site-packages')]
       ANARCI_LOC = os.path.join(sitepackages[0], 'anarci') # site-packages/ folder
...
setup(name='anarci',
     ...
     data_files = [ ('bin', ['bin/muscle', 'bin/muscle_macOS', 'bin/ANARCI', 'bin/ANARCI.bat', 'bin/muscle.exe']) ],
